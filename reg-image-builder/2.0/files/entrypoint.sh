...
#
KANIKO_IGNORE_VAR_RUN_DEFAULT=false

readonly KANIKO_DIGEST_BUILD_ARGS=${KANIKO_DIGEST_BUILD_ARGS}
readonly KANIKO_IGNORE_VAR_RUN=${KANIKO_IGNORE_VAR_RUN:-$KANIKO_IGNORE_VAR_RUN_DEFAULT}
...
function compare_images()
{
    ...
    then
        ...
        changes_apt=$(jq '.[] | select(.DiffType=="Apt")  | .Diff.InfoDiff | length' ${comparison_res_file})
...
function build_image()
{
    ...
    # || --ignore-var-run=false     || Fixes an error related to /var/run   || https://github.com/GoogleContainerTools/kaniko/issues/506
    ...
    executor --context ${BUILD_PATH} \
      ...
      --destination $TAG --ignore-var-run=${KANIKO_IGNORE_VAR_RUN} --cleanup --cache=false --log-timestamp $kaniko_args
