...
main()
{
    echo https://gitlab-ci-token:$CI_JOB_TOKEN@gitlab-forge.din.developpement-durable.gouv.fr > /root/.git-credentials
    git config --global credential.helper store

    # Récupère la liste des sous-dossiers (dépôts Git)
    subdirectories=$(find /usr/local/swarm_clusters/ -maxdepth 1 -type d)
