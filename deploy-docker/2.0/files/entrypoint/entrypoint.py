...
def find_docker_compose_paths(path) :
    tag = os.environ["TAG"]
...
def trigger_jenkins(clusters,image_info) :
    ...
    for cluster in clusters :
        additional_params = {"ref":image_info["image_name"],"stack":cluster["name"],"latest_docker_image_digest":image_info["latest_docker_image_digest"],"image_name":image_info["image_name_with_registry"]}
...
def main():
    ...
    docker_compose_paths = find_docker_compose_paths(PATH_GIT)
    ...
    if image_info != {} :
        trigger_jenkins(clusters,image_info)
